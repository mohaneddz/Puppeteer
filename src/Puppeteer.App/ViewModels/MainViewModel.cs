...
public sealed partial class MainViewModel:ObservableObject
{
 ...
 public BatchCollection<Project> Projects{get;}=[]; public ObservableCollection<string> Types{get;}=[]; public ObservableCollection<string> TechnologyOptions{get;}=[]; public ObservableCollection<string> CategoryOptions{get;}=[]; public IReadOnlyList<string> ProjectStatusOptions{get;}=["Not set", "Ongoing", "MVP", "Done", "Archived", "Cancelled"]; public IReadOnlyList<string> StatusFilterOptions{get;}=["All", "Not set", "Ongoing", "MVP", "Done", "Archived", "Cancelled"]; public ObservableCollection<RootFolder> Roots{get;}=[]; public ObservableCollection<TerminalSessionViewModel> Sessions{get;}=[]; public ObservableCollection<IconCandidate> IconCandidates{get;}=[];
 public IReadOnlyList<string> SpecialFilters{get;}=["All","Favorites","Running","Archived","Hidden","Git repositories","No Git repository","Git changes","Custom icon","Uncategorized","Never opened"];
 ...
 public string Search{get=>_search;set{if(Set(ref _search,value)){Raise(nameof(ActiveSearch));QueueSearchRefresh();}}} public string ActiveSearch{get=>_currentPage=="Docs"?_docSearch:_search;set{if(_currentPage=="Docs")DocSearch=value;else Search=value;}} public string SearchPlaceholder=>_currentPage=="Docs"?"Search docs...":"Search projects..."; public string SelectedType{get=>_selectedType;set{if(Set(ref _selectedType,value))Refresh();}} public string SelectedTechnology{get=>_selectedTechnology;set{if(Set(ref _selectedTechnology,value))Refresh();}} public string SelectedCategory{get=>_selectedCategory;set{if(Set(ref _selectedCategory,value))Refresh();}} public string SelectedStatus{get=>_selectedStatus;set{if(Set(ref _selectedStatus,value))Refresh();}} public string SelectedSpecial{get=>_selectedSpecial;set{if(Set(ref _selectedSpecial,value))Refresh();}} public string CurrentPage{get=>_currentPage;set{if(Set(ref _currentPage,value)){SavePref("LastPage",value);Raise(nameof(IsLibraryPage));Raise(nameof(ProjectPageTitle));Raise(nameof(ActiveSearch));Raise(nameof(SearchPlaceholder));Refresh();if(value=="Docs")RebuildDocEntries();}}} public bool IsLibraryPage=>_currentPage is "Projects"; public string ProjectPageTitle=>_currentPage; public Project? SelectedProject{get=>_selectedProject;set{if(_refreshing)return;if(!Set(ref _selectedProject,value))return;IconCandidates.Clear();Raise(nameof(HasMoreGitFiles));Raise(nameof(MoreGitFileCount));Raise(nameof(IconFillsContainer));Raise(nameof(IconShape));Raise(nameof(SelectedProjectStatus));_=LoadGitForAsync(value);}} public bool IconFillsContainer{get=>_selectedProject?.IconFill??false;set{if(_selectedProject is not { } project||project.IconFill==value)return;var updated=project with{IconFill=value};_selectedProject=updated;var index=_allProjects.FindIndex(p=>p.Id==updated.Id);if(index>=0)_allProjects[index]=updated;_=QueueWriteAsync(()=>_repository.SetProjectIconFillAsync(updated.Id,value));Raise(nameof(SelectedProject));Raise(nameof(IconFillsContainer));Refresh();}}
 ...
 public MainViewModel(IProjectRepository repository,IProjectBackupService backup,IProjectScanner scanner,ProjectSearchService searchService,ITerminalService terminalService,IProjectLauncher launcher,IIconDiscoveryService icons,IFilePicker picker,IProjectClassifier classifier,IDocMatcher docMatcher,[FromKeyedServices("groq")]IDocFieldGenerator groqFieldGenerator,[FromKeyedServices("claude")]IDocFieldGenerator claudeFieldGenerator,AppConfig config,IGitMetadataService git,IStartupService startup,IProjectDocVault vault)
 {
  _repository=repository;_backup=backup;_scanner=scanner;_searchService=searchService;_terminalService=terminalService;_launcher=launcher;_icons=icons;_picker=picker;_classifier=classifier;_docMatcher=docMatcher;_groqFieldGenerator=groqFieldGenerator;_claudeFieldGenerator=claudeFieldGenerator;_config=config;_git=git;_startup=startup;_vault=vault;InitializeDocs();InitializeFieldEditor();InitializeDialog();
  ...
  ClearFiltersCommand=new(_=>{_selectedFolder=null;Raise(nameof(SelectedFolder));Raise(nameof(FolderFilterActive));_selectedType="All";_selectedTechnology="All";_selectedCategory="All";_selectedStatus="All";_selectedSpecial="All";Raise(nameof(SelectedStatus));Raise(nameof(SelectedSpecial));_search="";Raise(nameof(Search));Raise(nameof(ActiveSearch));Refresh();});
 }
 ...
 private void Refresh()
 {
  ...
  try
  {
   ...
   IEnumerable<Project> visible=_selectedSpecial switch{"Archived"=>_allProjects.Where(p=>_archivedProjects.Contains(p.Id)&&!_hiddenProjects.Contains(p.Id)),"Hidden"=>_allProjects.Where(p=>_hiddenProjects.Contains(p.Id)),_=>_allProjects.Where(p=>!_archivedProjects.Contains(p.Id)&&!_hiddenProjects.Contains(p.Id))};
   ...
   var filtered=_searchService.Filter(scoped,Search,_selectedType,_selectedTechnology,_selectedCategory,running,status:_selectedStatus).Where(p=>_selectedSpecial switch{
