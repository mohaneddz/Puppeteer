...
public sealed class MainViewModel:ObservableObject
{
 ...
 private readonly List<Project> _allProjects=[]; private readonly HashSet<Guid> _hiddenProjects=[]; private readonly HashSet<Guid> _archivedProjects=[]; private string _search=""; private string _selectedType="All"; private string _selectedTechnology="All"; private string _selectedCategory="All"; private string _currentPage="Projects"; private Project? _selectedProject; private TerminalSessionViewModel? _selectedSession; private bool _terminalOpen; private bool _isBusy; private string _status="Ready"; private string _defaultShell="powershell.exe";
 ...
 private async Task DeleteProjectAsync(Project? project){if(project is null)return;if(Roots.Any(root=>string.Equals(Path.GetFullPath(root.Path).TrimEnd(Path.DirectorySeparatorChar),Path.GetFullPath(project.Path).TrimEnd(Path.DirectorySeparatorChar),StringComparison.OrdinalIgnoreCase))){Status="A root folder cannot be deleted as a project.";return;}var answer=MessageBox.Show($"Permanently delete the project folder?\n\n{project.Path}\n\nThis cannot be undone.","Delete project?",MessageBoxButton.YesNo,MessageBoxImage.Warning);if(answer!=MessageBoxResult.Yes)return;try{foreach(var session in Sessions.Where(s=>s.Session?.ProjectPath.Equals(project.Path,StringComparison.OrdinalIgnoreCase)==true).ToArray())CloseSession(session);if(Directory.Exists(project.Path))Directory.Delete(project.Path,true);await _repository.DeleteProjectsAsync([project.Id]);_allProjects.RemoveAll(p=>p.Id==project.Id);_hiddenProjects.Remove(project.Id);_archivedProjects.Remove(project.Id);RebuildTree();Refresh();SelectedProject=Projects.FirstOrDefault();Status=$"Deleted {project.Name} and its project files.";}catch(Exception e){Status=$"Couldn't delete {project.Name}: {e.Message}";}}
