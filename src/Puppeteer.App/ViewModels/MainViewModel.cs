...
public sealed class MainViewModel:ObservableObject
{
 ...
 private void OpenFolder(Project? project){if(project is null)return;if(Directory.Exists(project.Path)){_launcher.OpenFolder(project.Path);Status=$"Opened {project.Name} in Explorer";}else Status="Sample project — add a real root to open it.";}
 private void OpenIde(Project? project){if(project is null)return;if(Directory.Exists(project.Path)){_launcher.OpenInIde(project.Path);Status=$"Opening {project.Name} in your IDE";}else Status="Sample project — add a real root to open it.";}
